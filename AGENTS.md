...
- **Dialogs force light theme** - `OpenPlanFutureTaskDialogAsync` and `OpenCalendarViewDialogAsync` set `RequestedTheme = Light` (hardcoded ink text + native month grid wash out in dark mode); future dialog also opens its `CalendarDatePicker` flyout on `Loaded`.
