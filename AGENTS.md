...
- **Daily prompt is mandatory** - `DailyPromptManager` tracks `CurrentStreak`/`BestStreak`; `CheckDailyPromptAsync` runs once per calendar day (`ShouldShowPrompt` checks `LastPromptDate` != today) and is **required** — no Skip button, Cancel loops, window is forced visible via `ShowMainWindow`, and `HasIncompleteTasks` includes `IsDailyPlanMissing` so Focus stays `Locked` (blocking + hosts) until today's task is added, even with zero tasks. `InitializeAsync` auto-enables startup via `EnsureStartupEnabledAsync` (which always rewrites the HKCU Run key to the running exe, healing stale Debug paths) so the prompt actually appears at logon. Guarded by `dailyPromptActive` so startup + wake paths can't stack dialogs. `SkipToday` remains in code but is unused.
