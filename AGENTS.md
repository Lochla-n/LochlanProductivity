...
- **Tamper seal** - `TamperSeal.cs` HMACs tasks.json (DPAPI-protected key in `%LocalAppData%/LochlanProductivity/seal.key`); a `.seal` mismatch rejects the list and sets `taskListTampered`, which force-locks focus (`HasIncompleteTasks`), gates sync, and suppresses the daily prompt until a fresh plan is saved. Missing seal = trust-on-first-use (upgrade path). Accountability, not attacker-proofing.
- **Strict-mode gates** - Sync Now AND startup sync are denied while tasks are incomplete (a remote completion must not unlock this machine); all settings dialogs share the same lock. Browser closing on block engagement is opt-in (`AppSettingsManager.KillBrowsersOnEngage`, default off) - DNS-policy + hosts make it mostly redundant. Chromium secure-DNS is policy-disabled during enforcement (`BuiltInDnsClientEnabled=0` under HKCU Policies; HKCU\Policies is admin-ACLed so un-elevated writes stage through the helper task via `dns-policy-staging.txt`).
